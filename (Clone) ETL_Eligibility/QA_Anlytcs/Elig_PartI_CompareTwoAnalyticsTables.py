...
#************************************************************************************************************************************
#*                                                                                                                                  *
#*   NOTEBOOK:     Elig_PartI_CompareTwoAnalyticsTables.                                                                            *
#*                                                                                                                                  *
#*   DESCRIPTION:                                                                                                                   *
#*                                                                                                                                  *
#*                                                                                                                                  *
...
dbutils.widgets.text('DiffAnlysTblNm', 'EligAnlytcs_Part_I_DiffAnlys')
...
# MAGIC     ,COALESCE(PrevTbl.I_DTE_EFFECTIVE, CurntTbl.I_DTE_EFFECTIVE) AS I_DTE_EFFECTIVE -- This is the common identifier
...
# MAGIC     ,CASE WHEN PrevTbl.I_DTE_END                 != CurntTbl.I_DTE_END                 THEN PrevTbl.I_DTE_END                 ELSE NULL END AS CurntTbl_I_DTE_END                               
# MAGIC     ,CASE WHEN PrevTbl.I_DTE_END                 != CurntTbl.I_DTE_END                 THEN CurntTbl.I_DTE_END                ELSE NULL END AS PrevTbl_I_DTE_END                               
...
# MAGIC                                                               AND PrevTbl.I_DTE_EFFECTIVE = CurntTbl.I_DTE_EFFECTIVE
# MAGIC WHERE 
# MAGIC        PrevTbl.NUM_CASE                  != CurntTbl.NUM_CASE                  
# MAGIC     OR PrevTbl.Derived2                  != CurntTbl.Derived2                     
# MAGIC     OR PrevTbl.I_DTE_END                 != CurntTbl.I_DTE_END                
# MAGIC    AND PrevTbl.I_DTE_EFFECTIVE is not null
# MAGIC ;
...
# MAGIC   ),
# MAGIC   Cnt_I_DTE_END AS (
# MAGIC     SELECT COUNT(*) AS Diff_I_DTE_END
# MAGIC     FROM ${catalog}.${schema_name}.${DiffAnlysTblNm}
# MAGIC     WHERE CurntTbl_I_DTE_END IS NOT NULL
# MAGIC        OR PrevTbl_I_DTE_END IS NOT NULL
# MAGIC   )
# MAGIC
# MAGIC SELECT
...
# MAGIC   format_number(t3.Diff_I_DTE_END, 0)                       AS Diff_I_DTE_END,
# MAGIC   format_number(CASE WHEN t4.cntTotal = 0 THEN 0
# MAGIC                   ELSE t3.Diff_I_DTE_END * 100 / t4.cntTotal
# MAGIC              END, '###.#')                                  AS Pcntg_I_DTE_END,
# MAGIC   format_number(t4.cntTotal, 0)                             AS cntTotal
# MAGIC FROM
# MAGIC   Cnt_Derived2 t1,
# MAGIC   Cnt_NUM_CASE t2,
# MAGIC   Cnt_I_DTE_END t3,
# MAGIC   Sum_Total t4
# MAGIC ;
...
# MAGIC     FROM ${catalog}.${schema_name}.${CurntTblNm} CurntTbl
# MAGIC     WHERE TRUE
# MAGIC       AND CurntTbl.I_DTE_EFFECTIVE is not null
...
# MAGIC            AND CurntTbl.I_DTE_EFFECTIVE = t1.I_DTE_EFFECTIVE
# MAGIC        )
# MAGIC   ),
...
# MAGIC            AND PrevTbl.I_DTE_EFFECTIVE = t4.I_DTE_EFFECTIVE
# MAGIC        )
# MAGIC   )
