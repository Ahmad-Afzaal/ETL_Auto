...
#************************************************************************************************************************************
#*                                                                                                                                  *
#*   NOTEBOOK:     Elig_PartK_CompareTwoAnalyticsTables.                                                                            *
#*                                                                                                                                  *
#*   DESCRIPTION:                                                                                                                   *
#*                                                                                                                                  *
#*                                                                                                                                  *
...
dbutils.widgets.text('DiffAnlysTblNm', 'EligAnlytcs_Part_K_DiffAnlys')
...
# MAGIC     ,COALESCE(PrevTbl.K_DTE_EFFECTIVE, CurntTbl.K_DTE_EFFECTIVE) AS K_DTE_EFFECTIVE -- This is the common identifier
...
# MAGIC     ,CASE WHEN PrevTbl.K_DTE_END                 != CurntTbl.K_DTE_END                 THEN PrevTbl.K_DTE_END                 ELSE NULL END AS PrevTbl_K_DTE_END
# MAGIC     ,CASE WHEN PrevTbl.K_DTE_END                 != CurntTbl.K_DTE_END                 THEN CurntTbl.K_DTE_END                ELSE NULL END AS CurntTbl_K_DTE_END
...
# MAGIC                                                               AND PrevTbl.K_DTE_EFFECTIVE = CurntTbl.K_DTE_EFFECTIVE
# MAGIC WHERE 
# MAGIC        PrevTbl.NUM_CASE                  != CurntTbl.NUM_CASE                  
# MAGIC     OR PrevTbl.Derived2                  != CurntTbl.Derived2                     
# MAGIC     OR PrevTbl.K_DTE_END                 != CurntTbl.K_DTE_END                
# MAGIC    AND PrevTbl.K_DTE_EFFECTIVE is not null
# MAGIC ;
...
# MAGIC   ),
# MAGIC   Cnt_K_DTE_END AS (
# MAGIC     SELECT COUNT(*) AS Diff_K_DTE_END
# MAGIC     FROM ${catalog}.${schema_name}.${DiffAnlysTblNm}
# MAGIC     WHERE CurntTbl_K_DTE_END IS NOT NULL
# MAGIC        OR PrevTbl_K_DTE_END IS NOT NULL
# MAGIC   )
# MAGIC
# MAGIC SELECT
...
# MAGIC   format_number(t3.Diff_K_DTE_END, 0)                       AS Diff_K_DTE_END,
# MAGIC   format_number(CASE WHEN t4.cntTotal = 0 THEN 0
# MAGIC                   ELSE t3.Diff_K_DTE_END * 100 / t4.cntTotal
# MAGIC              END, '###.#')                                  AS Pcntg_K_DTE_END,
# MAGIC   format_number(t4.cntTotal, 0)                             AS cntTotal
# MAGIC FROM
# MAGIC   Cnt_Derived2 t1,
# MAGIC   Cnt_NUM_CASE t2,
# MAGIC   Cnt_K_DTE_END t3,
# MAGIC   Sum_Total t4
# MAGIC ;
...
# MAGIC     FROM ${catalog}.${schema_name}.${CurntTblNm} CurntTbl
# MAGIC     WHERE TRUE
# MAGIC       AND CurntTbl.K_DTE_EFFECTIVE is not null
...
# MAGIC            AND CurntTbl.K_DTE_EFFECTIVE = t1.K_DTE_EFFECTIVE
# MAGIC        )
# MAGIC   ),
...
# MAGIC            AND PrevTbl.K_DTE_EFFECTIVE = t4.K_DTE_EFFECTIVE
# MAGIC        )
# MAGIC   )
